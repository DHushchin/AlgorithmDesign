...
class BTree:
	...
	def search(self, key : int, current_node : Node = None):
		if current_node != None:
			low = 0  
			high = len(current_node.keys) - 1
			mid = 0
			while low <= high:
				mid = (high + low) // 2
				if current_node.keys[mid][0] == key:
					return (key, current_node.keys[mid][1])
				elif current_node.keys[mid][0] < key:  
					low = mid + 1  
				else:  
					high = mid - 1
			if current_node.leaf == False:
				return self.search(key, current_node.children[mid + 1])
			return None			
		else:
			#Search entire tree if node not provided
